...
public static class Extensions
{
    ...
    /// metrics and activities via <see cref="CachingTelemetry"/>.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public static IServiceCollection AddCachingV2(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        else
        {
            ...
            services.AddDataProtection()
                ...
                .SetApplicationName("MultiTenantStarter");
        }
    ...
    private static void DecorateHybridCache(IServiceCollection services)
    {
        var originalDescriptor = services.LastOrDefault(d => d.ServiceType == typeof(HybridCache)) 
            ?? throw new InvalidOperationException("HybridCache not registered .AddHybridCache must be called before DecorateHybridCache.");
        
        services.Remove(originalDescriptor);
        services.AddSingleton<HybridCache>(sp =>
        {
            HybridCache inner;
            if (originalDescriptor.ImplementationInstance is HybridCache hybridCache)
            {
                inner = hybridCache;
            }
