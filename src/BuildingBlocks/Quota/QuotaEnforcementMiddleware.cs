...
public sealed class QuotaEnforcementMiddleware : IMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        ...
        if (!_options.Enable || IsExempt(context))
        {
    ...
    private int? CalculateRetryAfter(DateTimeOffset? resultResetAtUtc)
    {
        if (resultResetAtUtc is null)
        {
            return null;
        }

        var delta = resultResetAtUtc.Value - _timeProvider.GetUtcNow();
