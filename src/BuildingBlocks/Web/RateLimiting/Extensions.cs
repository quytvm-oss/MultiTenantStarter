...
public static class Extensions
{
    ...
    private static PartitionedRateLimiter<HttpContext> CreateUserLimiter(RateLimitingOptions settings)
        => PartitionedRateLimiter.Create<HttpContext, string>(context =>
        {
            ...
            return string.IsNullOrWhiteSpace(user)
                ...
                : CreateFixedWindowPartition($"user:{user}", settings.User);
        });
    ...
    private static PartitionedRateLimiter<HttpContext> CreateTenantLimiter(RateLimitingOptions settings)
        => PartitionedRateLimiter.Create<HttpContext, string>(context =>
        {
            ...
            if (string.IsNullOrWhiteSpace(tenant))
                return RateLimitPartition.GetNoLimiter(AnonymousPartitionKey);

            // Lấy plan từ JWT claim
            var plan = context.User?.FindFirst(ClaimConstants.Plan)?.Value;
            var policy = settings.GetPlanPolicy(plan);
            return CreateFixedWindowPartition($"tenant:{tenant}", policy);
        });
    ...
    private static async ValueTask OnRejected(OnRejectedContext context, CancellationToken cancellationToken)
    {
        ...
        {
            httpContext.Response.Headers.RetryAfter = ((int)retryAfter.TotalSeconds).ToString(CultureInfo.InvariantCulture);
        }
