...
public sealed class ChannelAuditPublisher(
    ...
    : IAuditPublisher
{
    private static readonly IAuditScope DefaultScope = 
        new DefaultAuditScope(null, null, null, null, null, null, null, null, AuditTag.None);
    private readonly Channel<AuditEnvelope> _default = Channel.CreateBounded<AuditEnvelope>(new BoundedChannelOptions(capacity)
    {
    ...
    private readonly Channel<AuditEnvelope> _security = Channel.CreateBounded<AuditEnvelope>(new BoundedChannelOptions(securityCapacity)
    {
        AllowSynchronousContinuations = false,
        SingleReader = true,
        SingleWriter = false,
    ...
    public async ValueTask PublishAsync(IAuditEvent auditEvent, CancellationToken ct = default)
    {
        ...
        if (_default.Reader.Count >= capacity)
        {
