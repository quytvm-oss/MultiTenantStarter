    ...
    IConfiguration configuration)  : IDbInitializer
{
    ...
    private async Task AssignPermissionsToRoleAsync(IdentityDbContext dbContext, IReadOnlyList<Permission> permissions, Role role, CancellationToken cancellationToken = default)
    {
    ...
    private async Task SeedSystemGroupsAsync(CancellationToken cancellationToken)
    {
        ...
        if (adminRole is not null)
        {
            var existingGroupRole = await context.GroupRoles
                .AsNoTracking()
                .FirstOrDefaultAsync(gr => gr.RoleId == adminRole.Id && gr.GroupId == administratorsGroup.Id,
                    cancellationToken);

            if (existingGroupRole is null)
            {
                await context.GroupRoles.AddAsync(GroupRole.Create(administratorsGroup.Id,adminRole.Id), cancellationToken);
    ...
    private async Task SeedAdminUserAsync(CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(multiTenantContextAccessor.MultiTenantContext.TenantInfo?.Id) || 
            string.IsNullOrWhiteSpace(multiTenantContextAccessor.MultiTenantContext.TenantInfo?.AdminEmail))
            return;

        if (await userManager.Users.FirstOrDefaultAsync(
                u => u.Email == multiTenantContextAccessor.MultiTenantContext.TenantInfo!.AdminEmail, cancellationToken)
    ...
    private string ResolveInitialAdminPassword(string id)
    {
        var buffered = passwordBuffer.TryConsume(id);
        ...
        throw new InvalidOperationException(
            $"No initial admin password available for tenant '{id}'. " +
