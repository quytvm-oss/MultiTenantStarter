...
public class TenantThemeService : ITenantThemeService
{
    ...
    public TenantThemeService(HybridCache cache,
    ...
    public async Task SetAsDefaultThemeAsync(string tenantId, CancellationToken ct = default)
    {
        ...
        var existingDefault = await _dbContext.TenantThemes
            .FirstOrDefaultAsync(t => t.IsDefault, ct).ConfigureAwait(false);
    ...
    {
        if (assets.Logo?.Data is { Length: > 0 })
        {
        ...
        if (assets.LogoDark?.Data is { Length: > 0 })
        {
        ...
        if (assets.Favicon?.Data is { Length: > 0 })
        {
